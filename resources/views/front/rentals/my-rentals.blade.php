...
.rent-table tr:hover td { background: #fefefc; }
...
<div class="d-flex align-items-center justify-content-between mb-4">
    <div>
        <h2 style="font-size:1.6rem;font-weight:900;margin:0;color:#111;">Rental Bookings</h2>
        <p style="color:#888;font-size:.88rem;margin:0;">Manage your equipment hire requests, rental dates & view invoices</p>
    </div>
    <a href="{{ route('front.rentals') }}" class="btn btn-dark" style="border-radius:10px;font-weight:700;">
        <i class="bi bi-calendar-plus me-1"></i> Rent Equipment
    </a>
</div>
...
<div class="card-table">
    @if($bookings->isEmpty())
        <div style="text-align:center;padding:4rem 2rem;color:#888;">
            ...
            <h4 style="font-weight:800;color:#111;">No rental bookings yet</h4>
            <p>When you request equipment hire, your bookings and invoices will appear here.</p>
            <a href="{{ route('front.rentals') }}" class="btn btn-success mt-2" style="border:none;border-radius:10px;font-weight:700;">Browse Rentals</a>
        </div>
    @else
        <table class="rent-table">
            ...
            <tbody>
                ...
                <tr>
                    <td style="font-family:monospace;font-weight:800;color:#16a34a;">{{ $booking->booking_ref }}</td>
                    <td style="font-weight:700;">{{ $booking->listing->title ?? 'Listing Removed' }}</td>
                    <td style="font-size:.85rem;color:#555;">{{ \Carbon\Carbon::parse($booking->start_date)->format('d M Y') }} → {{ \Carbon\Carbon::parse($booking->end_date)->format('d M Y') }}</td>
                    <td style="font-weight:700;">{{ $booking->total_days }} day(s)</td>
                    <td style="font-weight:900;color:#111;">£{{ number_format($booking->total_amount, 2) }}</td>
                    <td><span class="badge-status badge-{{ $booking->status }}">{{ ucfirst($booking->status) }}</span></td>
                    <td>
                        <div class="d-flex gap-1 align-items-center">
                            <a href="/invoices/booking/{{ $booking->id }}" target="_blank" class="btn-inv" title="Download Rental Invoice">
                            ...
                            @if($booking->isPending() || $booking->isConfirmed())
                                <form action="{{ route('front.rentals.cancel', $booking->id) }}" method="POST" onsubmit="return confirm('Cancel this booking request?')">
                                    @csrf
                                    <button type="submit" class="btn btn-sm btn-outline-danger" style="border-radius:8px;font-size:.75rem;font-weight:700;">Cancel</button>
                                </form>
                            @endif
                        </div>
                    </td>
