...
<div class="page-header mb-4" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:1rem;">
    <div>
        <h1 class="page-title" style="font-size:1.6rem;font-weight:800;margin:0;">Create Rental Listing</h1>
        <p style="color:#888;font-size:.9rem;margin:.2rem 0 0 0;">Add film equipment to rent out on the Light As Air marketplace</p>
    </div>
...
<form method="POST" action="{{ route('vendor.rentals.store') }}" enctype="multipart/form-data">
    @csrf
    ...
    <div class="content-card mb-4">
        ...
        <div class="content-card-body">
            <div class="mb-3">
                ...
                <input type="text" name="title" class="form-control form-control-lg" value="{{ old('title') }}" required placeholder="e.g. ARRI Orbiter 30cm LED Fresnel Light — Complete Kit" style="border-radius:10px;font-weight:700;">
            </div>
            ...
            <div class="row g-3 mb-3">
                <div class="col-md-4">
                    ...
                    <input type="text" name="brand" class="form-control" value="{{ old('brand') }}" placeholder="e.g. ARRI, RED, Aputure" style="border-radius:10px;">
                </div>
                <div class="col-md-4">
                    <label class="form-label" style="font-size:.78rem;font-weight:700;text-transform:uppercase;color:#555;">Model Number</label>
                    <input type="text" name="model_number" class="form-control" value="{{ old('model_number') }}" placeholder="e.g. Orbiter 30" style="border-radius:10px;">
                </div>
                <div class="col-md-4">
                    <label class="form-label" style="font-size:.78rem;font-weight:700;text-transform:uppercase;color:#555;">Year Manufactured</label>
                    <input type="number" name="year_manufactured" class="form-control" value="{{ old('year_manufactured') }}" placeholder="{{ date('Y') }}" style="border-radius:10px;">
                </div>
            </div>

            <div class="mb-3">
                <label class="form-label" style="font-size:.78rem;font-weight:700;text-transform:uppercase;color:#555;">Short Summary</label>
                <input type="text" name="short_description" class="form-control" value="{{ old('short_description') }}" placeholder="Brief summary of included accessories or kit specs..." style="border-radius:10px;">
            </div>

            <div class="mb-3">
                <label class="form-label" style="font-size:.78rem;font-weight:700;text-transform:uppercase;color:#555;">Full Detailed Description <span class="text-danger">*</span></label>
                <textarea name="description" class="form-control" rows="6" required style="border-radius:10px;" placeholder="Provide complete details about the equipment, included accessories, cabling, flight cases, usage instructions, etc...">{{ old('description') }}</textarea>
            </div>
        </div>
    </div>
    ...
    <div class="content-card mb-4">
        ...
        <div class="content-card-body">
            <div class="mb-3">
                <label class="form-label" style="font-size:.78rem;font-weight:700;text-transform:uppercase;color:#555;">Upload Equipment Photos</label>
                <input type="file" name="images[]" id="rentalImagesInput" class="form-control form-control-lg" multiple accept="image/jpeg,image/png,image/webp" style="border-radius:10px;">
                <small class="text-muted mt-1 d-block">Select up to 10 high-quality photos (JPEG, PNG, WebP · Max 4 MB each). The first photo will be used as the primary image.</small>
            </div>
            <div id="rentalImagesPreview" class="row g-2 mt-2"></div>
        </div>
    </div>
        ...
        <div class="content-card-body">
            <div class="row g-3 mb-3">
                <div class="col-md-4">
                    ...
                    <input type="number" step="0.01" min="0.01" name="price_per_day" class="form-control" value="{{ old('price_per_day') }}" required placeholder="0.00" style="border-radius:10px;font-weight:700;">
                </div>
                <div class="col-md-4">
                    <label class="form-label" style="font-size:.78rem;font-weight:700;text-transform:uppercase;color:#555;">Weekly Rate (£)</label>
                    <input type="number" step="0.01" min="0" name="price_per_week" class="form-control" value="{{ old('price_per_week') }}" placeholder="Optional discount rate" style="border-radius:10px;">
                </div>
                <div class="col-md-4">
                    <label class="form-label" style="font-size:.78rem;font-weight:700;text-transform:uppercase;color:#555;">Security Deposit (£)</label>
                    <input type="number" step="0.01" min="0" name="deposit_amount" class="form-control" value="{{ old('deposit_amount', '0.00') }}" placeholder="0.00" style="border-radius:10px;">
                </div>
            </div>
            ...
            <div class="row g-3 mb-3">
                <div class="col-md-4">
                    ...
                    <input type="number" name="total_qty" class="form-control" value="{{ old('total_qty', 1) }}" min="1" required style="border-radius:10px;font-weight:700;">
                </div>
                <div class="col-md-4">
                    ...
                    <input type="number" name="min_rental_days" class="form-control" value="{{ old('min_rental_days', 1) }}" min="1" required style="border-radius:10px;">
                </div>
                <div class="col-md-4">
                    <label class="form-label" style="font-size:.78rem;font-weight:700;text-transform:uppercase;color:#555;">Max Rental Days</label>
                    <input type="number" name="max_rental_days" class="form-control" value="{{ old('max_rental_days') }}" placeholder="Leave blank for unlimited" style="border-radius:10px;">
                </div>
            </div>
                ...
                <div class="col-md-6">
                    ...
                    <div class="pt-2 d-flex align-items-center gap-3">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="offers_delivery" value="1" id="offersDelivery" @checked(old('offers_delivery'))>
                            <label class="form-check-label font-weight-bold" for="offersDelivery" style="font-size:.9rem;font-weight:700;">
                                Offer Delivery / Courier Shipping
                            </label>
                        </div>
                    </div>
                </div>
    ...
    <div style="display:flex;justify-content:flex-end;gap:.75rem;margin-top:1.5rem;">
        ...
        <button type="submit" class="btn-brand" style="border-radius:12px;padding:.8rem 2.2rem;font-size:.95rem;font-weight:800;display:inline-flex;align-items:center;gap:.5rem;">
            <i class="bi bi-send-check"></i> Submit Rental Listing for Approval
        </button>
    </div>
</form>

<script>
document.getElementById('rentalImagesInput')?.addEventListener('change', function(e) {
    const container = document.getElementById('rentalImagesPreview');
    container.innerHTML = '';
