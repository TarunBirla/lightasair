...
class RentalController extends Controller
{
    ...
    public function checkAvailability(Request $request, RentalListing $rental)
    {
        ...
        $total    = $subtotal + $rental->deposit_amount + ($request->delivery ? $rental->delivery_fee : 0);

        return response()->json([
            'available' => $available,
            'days'      => $days,
            'subtotal'  => number_format($subtotal, 2),
            'deposit'   => number_format($rental->deposit_amount, 2),
    ...
    public function book(Request $request, RentalListing $rental)
    {
        ...
        $validated = $request->validate([
            'start_date'      => 'required|date|after_or_equal:today',
            'end_date'        => 'required|date|after:start_date',
            'qty'             => 'required|integer|min:1|max:' . $rental->total_qty,
        ...
        if ($days < $rental->min_rental_days) {
        ...
        $delivery = $request->boolean('requires_delivery');
        $subtotal = $rental->price_per_day * $validated['qty'] * $days;
        $deliveryFee = $delivery ? $rental->delivery_fee : 0;
        $total    = $subtotal + $rental->deposit_amount + $deliveryFee;
        ...
        return redirect()->route('front.my-rentals')
                         ->with('success', 'Booking request sent! The vendor will confirm shortly.');
    }
